...
public class Dwarf extends MoveablePiece {
	
	private int numOfRescues;

	/**
	 * <b><p>Constructor</p></b>
	 * <p>Postconditions:</p> creates a new Dwarf and sets via set methods rank = 3,  
	 * 'team' team, numOfRescues = 0
	 * @param team is the team that Dwarf belongs
	 */
	...
	/**
	 * <b><p>Transformer</p></b>
	 * <p>Postconditions: the Dwarf attacks a Piece</p>
	...
	public int attack(Position position1, Position position2, Board board, Player player1, Player player2) {
		if ((board.getBoardElement(position2).getRank() == 1) || (board.getBoardElement(position2).getRank() == 2) || (board.getBoardElement(position2).getRank() == 0) || (board.getBoardElement(position2).getRank() == -1)) {
		...
		else if (board.getBoardElement(position2).getRank() == 3) {
			player1.lostPieces.add(board.getBoardElement(position1));
			board.setBoardElement(null, position1);
			player2.lostPieces.add(board.getBoardElement(position2));
	 ...
	 * <p>Postconditions: return the number of the rescues this Dwarf has done</p>
	 * @return the number of the rescues this Dwarf has done
	 */
	 ...
	 * <p>Postconditions: sets the number of rescues this Dwarf has done to numOfRescues</p>
	 * @param numOfRescues is the new number of rescues this Dwarf has done
	 */
